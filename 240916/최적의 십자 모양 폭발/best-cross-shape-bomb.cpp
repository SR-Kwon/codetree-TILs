...
int get_pair(vector<vector<int>> mat){
    int count = 0;
    for(int r = 0; r < n; r++){
        for(int c = 0; c < n; c++){
            if(mat[r][c] != 0){
...
int main() {
    ...
    vector<vector<int>> temp_mat(n, vector<int>(n));
    for(int i = 0; i < n; i++){
        for(int j = 0; j < n; j++){
            cin >> mat[i][j];
        }
    ...
    for(int r = 0; r < n; r++){
        for(int c = 0; c < n; c++){
            temp_mat = bomb(mat, r, c);
            temp_mat = grav(temp_mat);
            max = get_pair(temp_mat) > max ? get_pair(temp_mat) : max;
        }
    }
